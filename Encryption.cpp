		...
		for (int i = j; i < j + 8; i++)
		{
			...
			for (int x = 0; x <= 1; x++)
			{
				for (int z = 1; z <= 4;z++)
				{
					if (i <= 3)																			// Refactored
					{
						this->BitArrayV3[i][q++] = this->BitArrayV2[8 - ( 2 * z - x )][7 - 1 - 2 * i];
					}
					else
					{
						this->BitArrayV3[i][q++] = this->BitArrayV2[8 - (2 * z - x)][7 - (i % 4) * 2];
					}
...
void Encryption::selectInV(int j)
{
for(int h = 0; h < j + 8; h++)
	for (int i = j; i < j + 8; i++)
	{
		...
		for (int x = 0; x <= 1; x++)
		{
			for (int z = 1; z <= 4;z++)
			{
				if (i <= 3)																			// Refactored
				{
					this->BitArrayV2[8 - (2 * z - x)][7 - 1 - 2 * i] = this->BitArrayV3[h][i];
				}
				else
				{
					this->BitArrayV2[8 - (2 * z - x)][7 - (i % 4) * 2] = this->BitArrayV3[h][i];
				}
...
void Encryption::mirrorV(int j)
{
	...
	{ 
		...
		for (int n = 1; n <= 4; n++)
			for (int l = n - 1; l <= n; l++)
				 this->BitArray[5 * n -  4 * l - 1][i] = this->BitArrayV2[i][q++];
	}
...
void Encryption::mirror(int j)
{
	...
	{ 
		...
		for (int n = 1; n <= 4; n++)
			for (int l = n - 1; l <= n; l++)
				this->BitArrayV2[q++][i] = this->BitArray[5 * n -  4 * l - 1][i];
	}
...
void Encryption::printBitV3()
{
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			this->BitArray[i][j] = this->BitArrayV3[i][j];
...
void Encryption::copy()
{
	for (int i = 0; i < 8; i++)
	{
		...
		for (int j = 7; j >= 0; j--)
		{ 
			this->BitArray[i][8 - l++] = this->BitArrayV3[i][j];
		
		}

	}
}
void Encryption::inversepermutate()
{
	int q = -1;
	...
	for (int x = 1; x >= 0; x--)
	{
		...
		for (int i = 1; i <= 4; i++)
		{
			...
			for (int j = 0; j <= 7; j++)
			{
				if (x == 1)
				{
					this->BitArray[q][j] = this->BitArrayV2[2 * i - x][j];

				}
				else
				{
					this->BitArray[q][j] = this->BitArrayV2[2 * i - 2][j]; 
				}
			}
